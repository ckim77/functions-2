...
const evenNumbers = mixedNumbers.filter(x => x % 2 == 0);
...
const totalPopulation = populations.reduce((x,y) => x + y)
...
const myStrongest = monstersInYourPocket.filter(x => x.CP > 200);
...
/*
  Use a higher order method to get all the order totals after adding in the sales tax (given to you as a tax rate, hint: you'll need 
  to do some multiplication). Your answer should be an array of numbers, one total for each order.
  */

const orderTotals = orders.map(x => +((x.price * (x.tax + 1)).toFixed(2)));
